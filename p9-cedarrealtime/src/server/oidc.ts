...
const p9Scopes = ["chat.access"] as const;
...
function storeTokens(
  ...
  if (
    ...
    !scope?.split(/\s+/u).includes("chat.access")
  ) {
    throw new Error("The token response did not contain the required P9 grant");
  }
  return {
    issuer,
    subject,
  ...
  return {
    async authorizationUrl(transaction, loginHint) {
      return oidc.buildAuthorizationUrl(client, {
        ...
        response_type: "code",
        scope: ["openid", "profile", "offline_access", ...p9Scopes].join(" "),
